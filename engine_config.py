...
class EngineConfig:
    ...
    growth_enabled: bool = field(default_factory=lambda: os.getenv("DIGITAL_GROWTH_AUDIT_ENABLED", "true" if os.getenv("APP_ENV") == "production" else "false").lower() == "true")
    seo_pages: int = field(default_factory=lambda: bounded("SEO_MAX_PAGES_PER_SITE", 6, 1, 10))
    seo_links: int = field(default_factory=lambda: bounded("SEO_MAX_LINK_CHECKS", 12, 0, 50))
    seo_resource_seconds: int = field(default_factory=lambda: bounded("SEO_RESOURCE_BUDGET_SECONDS", 12, 1, 30))
    seo_services: int = field(default_factory=lambda: bounded("SEO_MAX_SERVICE_PAGES", 5, 0, 5))
    seo_locations: int = field(default_factory=lambda: bounded("SEO_MAX_LOCATION_PAGES", 5, 0, 5))
